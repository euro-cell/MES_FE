...
import styles from '../../../styles/production/status/StatusProjectList.module.css';
...
export default function StatusProjectList() {
  const [projects, setProjects] = useState<StatusProject[]>([]);
  const [loading, setLoading] = useState(true);
  const navigate = useNavigate();

  const loadProjects = async () => {
    try {
      const data = await getStatusProjects();
      setProjects(data);
    } catch (err) {
