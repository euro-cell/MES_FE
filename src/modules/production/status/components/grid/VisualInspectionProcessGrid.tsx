...
interface VisualInspectionProcessGridProps extends ProcessGridProps {
  processKey: string;
  processData: VisualInspectionProcessData;
}

export default function VisualInspectionProcessGrid({
  processKey,
  processData,
  daysInMonth,
  onTargetChange,
}: VisualInspectionProcessGridProps) {
    ...
    <React.Fragment>
      ...
      <tr>
        ...
        <td rowSpan={totalRowSpan} style={{ borderBottom: '2px solid #9ca3af' }}>
          ...
          {onTargetChange && (
            <button onClick={() => onTargetChange('visualInspection')} style={CHANGE_BUTTON_STYLE}>
              변경
            </button>
          )}
        </td>
        ...
        if (idx === 0) {
          return (
            <tr key={`${processKey}-ncr-${subType}`}>
              ...
              <td className={styles.subTypeLabel}>
                {VISUAL_INSPECTION_NCR_SUBTYPE_LABELS[subType]}
              </td>
              {Array.from({ length: daysInMonth }, (_, i) => {
                const day = i + 1;
                const dayData = dailyDataMap[day];
