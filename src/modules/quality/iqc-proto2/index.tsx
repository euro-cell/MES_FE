...
import { uploadIQCProto2Xlsx } from '../../../api/quality/IQCProto2Service';
 ...
 * IQC 열람 용도로는 문제없다.
 * 백엔드: POST /quality/iqc-proto2/upload (multipart, xlsx) -> { viewerUrl, fileName }
 */
export default function IQCProto2Index() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await uploadIQCProto2Xlsx(file);
      setViewerUrl(result.viewerUrl);
      setFileName(result.fileName);
      setStatus('ready');
    } catch (err) {
      console.error('IQC Proto2 xlsx 업로드/뷰어 조회 실패:', err);
  ...
  return (
    <div style={{ padding: 16 }}>
      <h2>IQC 프로토타입2 (Univer CLI daemon 뷰어 iframe 실험)</h2>
