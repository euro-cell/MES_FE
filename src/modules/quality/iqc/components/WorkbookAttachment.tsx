...
import {
  uploadIQCWorkbook,
  exportIQCWorkbookXlsx,
  getIQCWorkbook,
  fetchWorkbookDataFromUrl,
} from '../../../../api/quality/IQCWorkbookService';
...
interface WorkbookAttachmentProps {
  /** 저장된 IQC 검사 항목의 id. undefined면 아직 저장되지 않은 신규 항목이므로 업로드를 막는다. */
  iqcId: number | undefined;
}

/**
 * IQC 검사 항목에 원본 엑셀을 보관용으로 첨부/열람하는 컴포넌트.
 * 셀-필드 매핑은 하지 않으며, 검사 원본을 그대로 보여주는 용도로만 쓰인다.
 * xlsx 업로드 → 백엔드가 Univer CLI로 변환한 워크북 JSON을 Univer SDK로 렌더링.
 * iqcId 1건당 워크북 1개만 유지(재업로드 시 이전 첨부는 교체됨).
 */
export default function WorkbookAttachment({ iqcId }: WorkbookAttachmentProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const latest = await getIQCWorkbook(iqcId);
        if (cancelled) return;
        ...
        if (cancelled) return;
        console.error('IQC 워크북 조회 실패:', err);
        setErrorMsg(getErrorMessage(err, '첨부된 워크북을 불러오지 못했습니다.'));
        setStatus('error');
      } finally {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !containerRef.current || !iqcId) return;

    setStatus('loading');
    setErrorMsg('');

    try {
      const workbookData = await uploadIQCWorkbook(iqcId, file);
      workbookDataRef.current = workbookData;
      await renderWorkbook(workbookData, toggles);
      setSavedInfo({ fileName: file.name, uploadedAt: new Date().toISOString() });
      setStatus('ready');
    } catch (err) {
      console.error('IQC 워크북 업로드/렌더링 실패:', err);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const { blob, filename } = await exportIQCWorkbookXlsx(iqcId, workbookData);
      ...
      console.error('IQC 워크북 내보내기 실패:', err);
  ...
  return (
    <div style={{ padding: 16, border: '1px solid #e5e7eb', borderRadius: 8, marginTop: 16 }}>
      ...
      >
        <h4 style={{ margin: 0 }}>검사 원본 첨부 (보관용)</h4>
      ...
      {expanded && (
        <div style={{ marginTop: 12 }}>
          {!iqcId ? (
            <p style={{ color: '#888', fontSize: 13 }}>먼저 저장한 뒤 원본 엑셀을 첨부할 수 있습니다.</p>
          ) : (
            <>
              <p style={{ color: '#666', fontSize: 13 }}>
                검사 원본 엑셀 파일을 첨부해 보관/열람할 수 있습니다. 셀 값/서식/PNG·JPEG 이미지는 재현되지만 EMF
                이미지와 PDF 첨부(OLE 임베디드 객체)는 재현되지 않습니다. 항목당 최신 1개 파일만 유지됩니다.
              </p>
