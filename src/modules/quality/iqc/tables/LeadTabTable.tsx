...
const getDefaultResults = (): IQCResult[] => [
  // 너비
  { category: '너비', item: 'Al',       unit: 'mm', spec: '60.0±0.3',  refCoa: '', refLastData: 'N/A', sample1: '', sample2: '', sample3: '', isPassed: null, note: 'ⓑ' },
  { category: '너비', item: 'Sealant',  unit: 'mm', spec: '70.0±0.5',  refCoa: '', refLastData: 'N/A', sample1: '', sample2: '', sample3: '', isPassed: null, note: 'ⓖ' },
  { category: '너비', item: 'Margin',   unit: 'mm', spec: '5.0±0.5',   refCoa: '', refLastData: 'N/A', sample1: '', sample2: '', sample3: '', isPassed: null, note: 'ⓒ sealant' },
  // 길이
  { category: '길이', item: 'Al(pitch)',     unit: 'mm', spec: '36.0±0.5', refCoa: '', refLastData: 'N/A', sample1: '', sample2: '', sample3: '', isPassed: null, note: 'ⓕ' },
  { category: '길이', item: 'Weld part',     unit: 'mm', spec: '5.0±0.5',  refCoa: '', refLastData: 'N/A', sample1: '', sample2: '', sample3: '', isPassed: null, note: 'ⓗ head' },
  { category: '길이', item: 'Sealant',       unit: 'mm', spec: '12.0±0.5', refCoa: '', refLastData: 'N/A', sample1: '', sample2: '', sample3: '', isPassed: null, note: 'ⓐ' },
  { category: '길이', item: 'Sealant(Al)',   unit: 'mm', spec: '12.0±0.5', refCoa: '', refLastData: 'N/A', sample1: '', sample2: '', sample3: '', isPassed: null, note: 'ⓘ' },
  // 두께
  { category: '두께', item: 'Al',          unit: 'mm', spec: '0.30±0.02', refCoa: '', refLastData: 'N/A', sample1: '', sample2: '', sample3: '', isPassed: null, note: 'ⓓ' },
  { category: '두께', item: 'Sealant(Al)', unit: 'mm', spec: '0.50±0.04', refCoa: '', refLastData: 'N/A', sample1: '', sample2: '', sample3: '', isPassed: null, note: 'ⓔ total' },
];

const getDefaultCoaRefs = (): IQCCoaRef[] => [
  { attrName: 'Al Metal',               attrValue: '' },
  { attrName: 'Vender of insulation',   attrValue: '' },
  { attrName: '', attrValue: '' },
  { attrName: '', attrValue: '' },
  { attrName: '', attrValue: '' },
  { attrName: '', attrValue: '' },
...
const LeadTabTable: React.FC<LeadTabTableProps> = ({ data, onSave }) => {
  ...
  useEffect(() => {
    if (isEditing) getMaterialsByCategory('리드탭').then(setMaterials);
  }, [isEditing]);
  ...
  const handleTypeChange = (type: string) => {
    ...
    if (autoName) {
      ...
      if (autoCompany) {
        getMaterialLots({ category: '리드탭', type, isZeroStock: true }).then((res) => {
  ...
  const handleNameChange = (name: string) => {
    ...
    if (autoCompany) {
      getMaterialLots({ category: '리드탭', type: selectedType, isZeroStock: true }).then((res) => {
  ...
  const renderResultCells = (result: IQCResult, index: number, passDisplay: { text: string; color: string }) => (
    <>
      ...
      <td style={{ color: '#94a3b8' }}>{isEditing ? <input type="text" value={String(result.refLastData ?? '')} onChange={(e) => updateResult(index, 'refLastData', e.target.value)} className={styles.tableInput} /> : (result.refLastData || 'N/A')}</td>
          ...
          rows.push(
            <tr key={actualIndex}>
              ...
              <td className={styles.subItemCell}>
                {isEditing
                  ? <input type="text" value={result.item || ''} onChange={(e) => updateResult(actualIndex, 'item', e.target.value)} className={styles.tableInput} />
                  : result.item || ''}
              </td>
              {renderResultCells(result, actualIndex, passDisplay)}
            </tr>
          );
        });
        i = j;
      } else {
        const passDisplay = getPassDisplay(current.isPassed);
        rows.push(
          <tr key={i}>
            <td colSpan={2} className={styles.itemCell}>
              {isEditing
                ? <input type="text" value={current.category} onChange={(e) => updateResult(i, 'category', e.target.value)} className={styles.tableInput} />
                : current.category}
            </td>
            {renderResultCells(current, i, passDisplay)}
          </tr>
        );
        i++;
      }
  ...
  return (
    <div className={styles.tableContainer}>
            ...
            <button className={styles.specButton} onClick={() => {
                if (data) {
                  ...
                  if (data.type && data.name) {
                    getMaterialLots({ category: '리드탭', type: data.type, isZeroStock: true }).then((res) => {
      ...
      {/* 모식도면 */}
      <div className={styles.tableTitleRow} style={{ marginTop: '16px' }}>
        <h3 className={styles.tableTitle}>■ Al-Tab 모식도면</h3>
      </div>
