...
const AnodeMaterialTable: React.FC<AnodeMaterialTableProps> = ({ data, onSave }) => {
  ...
  const handleTypeChange = (type: string) => {
    ...
    if (autoName) {
      ...
      if (autoCompany) {
        getMaterialLots({ category: '음극재', type, isZeroStock: true }).then((res) => {
  ...
  const handleNameChange = (name: string) => {
    ...
    if (autoCompany) {
      getMaterialLots({ category: '음극재', type: selectedType, isZeroStock: true }).then((res) => {
            ...
            <button className={styles.specButton} onClick={() => {
                if (data) {
                  ...
                  if (data.type && data.name) {
                    getMaterialLots({ category: '음극재', type: data.type, isZeroStock: true }).then((res) => {
