...
const getDefaultResults = (): IQCResult[] => [
  { category: '두께', unit: '㎛', spec: '153±15', refCoa: '', refLastData: '', sample1: '', sample2: '', sample3: '', isPassed: null },
  { category: '너비', unit: 'mm', spec: '400±1',  refCoa: '', refLastData: '', sample1: '', sample2: '', sample3: '', isPassed: null },
];
...
/** 샘플1~3, CoA, Last data, 평균이 모두 동일한 값인지 확인 */
const isPerfectMatch = (result: IQCResult): boolean => {
  const vals = [result.refCoa, result.refLastData, result.sample1, result.sample2, result.sample3, result.average];
  const nonEmpty = vals.filter((v) => v !== '' && v !== null && v !== undefined);
  if (nonEmpty.length < 2) return false;
  return nonEmpty.every((v) => String(v) === String(nonEmpty[0]));
};

const PouchTable: React.FC<PouchTableProps> = ({ data, onSave }) => {
  ...
  useEffect(() => {
    if (isEditing) getMaterialsByCategory('파우치').then(setMaterials);
  }, [isEditing]);
      ...
      <table className={styles.iqcTable} style={{ marginTop: '-1px' }}>
        ...
        <tbody>
          {results.map((result, i) => {
            ...
            const perfectMatch = !isEditing && isPerfectMatch(result);
            return (
              <tr key={i}>
                ...
                <td>
                  {isEditing ? (
                    <input type="text" value={result.note ?? ''} onChange={(e) => updateResult(i, 'note', e.target.value)} className={styles.tableInput} />
                  ) : perfectMatch ? (
                    <span style={{ color: '#2563eb', fontWeight: 600, fontSize: '12px' }}>전 샘플 일치</span>
                  ) : (result.note ?? '')}
                </td>
              </tr>
            );
