...
interface ConductiveAdditive2TableProps {
  data?: IQCItem;
  projectId: number;
...
const getDefaultResults = (): IQCResult[] => [
  { category: '수분', unit: '%', spec: '≤0.3', refCoa: '', refLastData: '', sample1: '', sample2: '', sample3: '', isPassed: null, note: 'ASTM D1509' },
];

const getDefaultCoaRefs = (): IQCCoaRef[] => [
  { attrName: 'BET(㎡/g)', attrValue: '' },
  { attrName: 'pH', attrValue: '' },
  { attrName: 'Fe(ppm)', attrValue: '' },
  { attrName: 'Ni(ppm)', attrValue: '' },
  { attrName: '', attrValue: '' },
  { attrName: '', attrValue: '' },
  { attrName: '', attrValue: '' },
  { attrName: '', attrValue: '' },
...
const ConductiveAdditive2Table: React.FC<ConductiveAdditive2TableProps> = ({ data, onSave }) => {
  ...
  useEffect(() => {
    if (isEditing) getMaterialsByCategory('도전재').then(setMaterials);
  }, [isEditing]);
...
export default ConductiveAdditive2Table;
