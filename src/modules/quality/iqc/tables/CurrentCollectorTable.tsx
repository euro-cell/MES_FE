...
const getDefaultResults = (): IQCResult[] => [
  { category: '두께', unit: '㎛',   spec: '12±0.5',    refCoa: '', refLastData: '', sample1: '', sample2: '', sample3: '', isPassed: null, note: '' },
  { category: '너비', unit: 'mm',   spec: '230±1',     refCoa: '', refLastData: '', sample1: '', sample2: '', sample3: '', isPassed: null, note: '' },
  { category: 'Loading', unit: 'mg/㎠', spec: '3.24±0.05', refCoa: '', refLastData: '', sample1: '', sample2: '', sample3: '', isPassed: null, note: '' },
];
...
const CurrentCollectorTable: React.FC<CurrentCollectorTableProps> = ({ data, onSave }) => {
  ...
  useEffect(() => {
    if (isEditing) getMaterialsByCategory('집전체').then(setMaterials);
  }, [isEditing]);
  ...
  const handleTypeChange = (type: string) => {
    ...
    if (autoName) {
      ...
      if (autoCompany) {
        getMaterialLots({ category: '집전체', type, isZeroStock: true }).then((res) => {
  ...
  const handleNameChange = (name: string) => {
    ...
    if (autoCompany) {
      getMaterialLots({ category: '집전체', type: selectedType, isZeroStock: true }).then((res) => {
            ...
            <button className={styles.specButton} onClick={() => {
                if (data) {
                  ...
                  if (data.type && data.name) {
                    getMaterialLots({ category: '집전체', type: data.type, isZeroStock: true }).then((res) => {
...
export default CurrentCollectorTable;
