...
const getDefaultResults = (): IQCResult[] => [
  { category: '두께',                    unit: '㎛',  spec: '12.0±1.5',   refCoa: '', refLastData: '', sample1: '', sample2: '', sample3: '', isPassed: null, note: '' },
  { category: '너비',                    unit: 'mm',  spec: '192.0±0.3',  refCoa: '', refLastData: '', sample1: '', sample2: '', sample3: '', isPassed: null, note: '' },
  { category: 'Shrinkage (130℃/1hr)', item: 'MD', unit: '%',  spec: '≤5.0', refCoa: '', refLastData: '', sample1: '', sample2: '', sample3: '', isPassed: null, note: '' },
  { category: 'Shrinkage (130℃/1hr)', item: 'TD', unit: '%',  spec: '≤5.0', refCoa: '', refLastData: '', sample1: '', sample2: '', sample3: '', isPassed: null, note: '' },
];
...
const SeparatorTable: React.FC<SeparatorTableProps> = ({ data, onSave }) => {
  ...
  useEffect(() => {
    if (isEditing) getMaterialsByCategory('분리막').then(setMaterials);
  }, [isEditing]);
  ...
  const handleTypeChange = (type: string) => {
    ...
    if (autoName) {
      ...
      if (autoCompany) {
        getMaterialLots({ category: '분리막', type, isZeroStock: true }).then((res) => {
  ...
  const handleNameChange = (name: string) => {
    ...
    if (autoCompany) {
      getMaterialLots({ category: '분리막', type: selectedType, isZeroStock: true }).then((res) => {
  ...
  /** category+item 구조에서 rowspan 처리하여 행 렌더링 */
  const renderInspectionRows = () => {
    const rows: React.JSX.Element[] = [];
    let i = 0;
    while (i < results.length) {
      const current = results[i];
      // 같은 category이고 item이 있는 행들을 그룹으로 묶음
            ...
            <button className={styles.specButton} onClick={() => {
                if (data) {
                  ...
                  if (data.type && data.name) {
                    getMaterialLots({ category: '분리막', type: data.type, isZeroStock: true }).then((res) => {
