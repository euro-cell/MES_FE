...
const ElectrolyteTable: React.FC<ElectrolyteTableProps> = ({ data, onSave }) => {
  ...
  const handleTypeChange = (type: string) => {
    ...
    if (autoName) {
      ...
      if (autoCompany) {
        getMaterialLots({ category: '전해액', type, isZeroStock: true }).then((res) => {
  ...
  const handleNameChange = (name: string) => {
    ...
    if (autoCompany) {
      getMaterialLots({ category: '전해액', type: selectedType, isZeroStock: true }).then((res) => {
            ...
            <button className={styles.specButton} onClick={() => {
                if (data) {
                  ...
                  if (data.type && data.name) {
                    getMaterialLots({ category: '전해액', type: data.type, isZeroStock: true }).then((res) => {
