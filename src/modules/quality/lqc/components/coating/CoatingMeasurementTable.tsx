...
const calculateN = (rows: MeasurementRow[]): number => {
...
export default function CoatingMeasurementTable({
  projectId,
  usl,
  lsl,
  onNChange,
  onDataChange,
}: CoatingMeasurementTableProps) {
        ...
        <table className={styles.lqcTable}>
          <thead>
            <tr>
              ...
              {Array.from({ length: 8 }, (_, i) => (
                <th key={i} className={i === 0 ? styles.groupBorder : undefined}>
                  X{i + 1}
                </th>
              ))}
          ...
          <tbody>
            {rows.length > 0 ? (
              ...
              <tr>
                <td colSpan={20} className={styles.noDataRow}>
                  데이터 없음
                </td>
              </tr>
            )}
