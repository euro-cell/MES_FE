...
import type { CoatingAnodeMeasurementRow } from './CoatingAnodeMeasurementTable';
import styles from '../../../../../styles/quality/lqc/LQCTable.module.css';

ChartJS.register(CategoryScale, LinearScale, PointElement, LineElement, Title, Tooltip, Legend);

interface CoatingAnodeXbarChartProps {
  data: CoatingAnodeMeasurementRow[];
}

export default function CoatingAnodeXbarChart({ data }: CoatingAnodeXbarChartProps) {
  ...
  const dataValues = valid.flatMap(d => [d.xbar, d.xbar_cl, d.xbar_ucl, d.xbar_lcl]).filter((v): v is number => v !== null && !isNaN(v));
  const dataMin = Math.min(...dataValues);
  const dataMax = Math.max(...dataValues);
  let step = tolerance <= 0.1 ? 0.02 : tolerance <= 0.3 ? 0.05 : tolerance <= 1 ? 0.1 : tolerance <= 3 ? 0.5 : 1;
  let halfSteps = Math.ceil(Math.max(tolerance, target - dataMin, dataMax - target) / step) + 1;
  while (halfSteps * 2 + 1 > 7) { step *= 2; halfSteps = Math.ceil(Math.max(tolerance, target - dataMin, dataMax - target) / step) + 1; }
  ...
  const makeOptions = (fontSize: number) => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Xbar Chart _ Anode Coating',
        font: { size: fontSize },
      },
      tooltip: {
        callbacks: {
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <span style={{ fontWeight: 600, fontSize: 15 }}>Xbar Chart _ Anode Coating</span>
              <button
                onClick={() => setExpanded(false)}
                style={{
                  padding: '4px 12px',
