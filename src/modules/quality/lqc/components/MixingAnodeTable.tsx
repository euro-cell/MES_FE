...
interface MixingAnodeTableProps {
  projectId: number;
}
...
export default function MixingAnodeTable({ projectId }: MixingAnodeTableProps) {
  // 규격 모달 상태
  ...
  useEffect(() => {
    const loadSpecs = async () => {
      try {
        const specs = await getLQCSpecs(projectId, 'MIXING_ANODE');
  ...
  useEffect(() => {
    const loadBinderData = async () => {
      try {
        const data = await getLQCBinderData(projectId, 'A'); // 음극(Anode)
        setBinderData(data);
      } catch (err) {
  ...
  const handleSaveSpecs = async (specs: Record<string, SpecValue>) => {
    ...
    try {
      await saveLQCSpec(projectId, 'MIXING_ANODE', itemType, specs);
  ...
  useEffect(() => {
    const loadSlurryData = async () => {
      try {
        const data = await getLQCSlurryData(projectId, 'A'); // 음극(Anode)
        setSlurryData(data);
      } catch (err) {
    ...
    <div className={styles.tableContainer}>
      ...
      <div className={styles.tableSection}>
        <div className={styles.tableTitleRow}>
          <h3 className={styles.tableTitle}>Binder Solution Mixing 검사 (음극)</h3>
      ...
      <div className={styles.tableSection}>
        <div className={styles.tableTitleRow}>
          <h3 className={styles.tableTitle}>Slurry Mixing 검사 (음극)</h3>
