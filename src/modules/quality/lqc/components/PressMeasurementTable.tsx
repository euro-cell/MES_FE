...
function toPressMeasurementRows(data: PressData[]): PressMeasurementRow[] {
...
export default function PressMeasurementTable({ projectId, onNChange, onDataChange }: PressMeasurementTableProps) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const converted = toPressMeasurementRows(data);
        setRows(converted);
