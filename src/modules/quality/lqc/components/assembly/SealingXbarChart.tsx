...
interface SealingXbarChartProps {
  title: string;
  yMin: number;
  yMax: number;
  yStep: number;
...
export default function SealingXbarChart({ title, yMin, yMax, yStep, data }: SealingXbarChartProps) {
  ...
  const tickCount = Math.round((yMax - yMin) / yStep) + 1;
  const yTicks = Array.from({ length: tickCount }, (_, i) =>
    parseFloat((yMin + i * yStep).toFixed(2))
  );

  const xbarColors = valid.map(row =>
    row.xbar > row.xbar_ucl || row.xbar < row.xbar_lcl ? '#FF0000' : '#80350E'
  );
